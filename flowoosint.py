...
def _set_log_file(path: str):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(f"FlowOsint v3.0 — Live Log\nStarted: {datetime.now()}\n{'='*60}\n\n")
...
def mod_hibp(domain, session):
    ...
    try:
        r = rs.get(url, timeout=rs._fo_timeout, verify=False,
                   headers={"User-Agent": "FlowOsint/3.0 OSINT-Tool"})
    except Exception as e:
...
def save_report_full(target, all_results, scored, outfile=None):
    ...
    lines += [
        ...
        "",
        f"> **Generated by FlowOsint v3.0** — for authorized security research only",
        "",
        "---",
        "",
    ...
    lines += [
        ...
        "---",
        "",
        f"*Report generated by FlowOsint v3.0 on {datetime.now().strftime('%Y-%m-%d at %H:%M')}*",
...
def show_info():
    ...
    print(f"\n  {BR}FlowOsint v3.0{RE} — Module Reference\n")
